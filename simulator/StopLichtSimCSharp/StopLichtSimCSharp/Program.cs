...
namespace StopLichtSimCSharp
{
    ...
    {       
        static void Main()
        {
            ...
            Raylib.InitWindow(800, 800, "like honestly pretty alright trafficlight sim, definitely one of the most simulators of all time");
                ...
                if (nodeDevMode) //change to true if you want the nodes rendered
                {
                    foreach (Lane lane in Lanes)
                    {
                        foreach (var node in lane.CheckPointNodes)
                        {
                            ...
                            //963
                            if (new[] { 9, 6, 36, 33, 67,70, 132,138, 158,161,231,234,277,280,312,315,348,351,369,372,406,409,439,442,466,469,485,488,555,558, 626, 627, 638, 639,  642,  656, 681,682,684,685,688, 689,  753,754, 792, 811,820, 822, 862, 916, 920, 921, 928, 931,933, 934, 940, 941, 967, 968, 975, 987, 795, 1032, 1033, 1035,1036, 1037,1040,1049,1100, 1101,1111, 1113, 1200, 1201,1483, 1683,1685, 1978, 9998,9999, 10032 }.Contains(node.NodeID))
                            {
